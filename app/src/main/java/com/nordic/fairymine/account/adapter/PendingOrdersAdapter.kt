...
import com.nordic.fairymine.databinding.RowAccountOrderPendingBinding
 ...
 */
class PendingOrdersAdapter(private val listener: PendingOrderListener) : BaseListAdapter<Offer>() {
    ...
    override fun getContentViewHolder(context: Context?, root: ViewGroup?): DataBindViewHolder<*> =
            DataBindViewHolder(RowAccountOrderPendingBinding
    ...
    override fun onBindViewHolder(holder: DataBindViewHolder<*>, position: Int) {
        ...
        when (holder.binding) {
            is RowAccountHeaderBinding -> holder.binding.label = context.getString(R.string.account_orders_pending_header_title)
            ...
            is RowAccountOrderPendingBinding -> {
                val offer = getItemAt(position - 1)
                holder.binding.viewModel = AccountEntityVM(offer, { listener.onResendPendingTransaction(it) })
            }
        }
    }
